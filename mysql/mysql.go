...
// DB db handle
type DB struct {
	transactions map[int]*Transation
...
// .eg DB.Count()
// .eg DB.Count("id")
func (d *DB) Count(params ...string) (int, error) {
	var (
		err   error
		count int
		field = DefalutFields
...
func (d *DB) Begin() (*DB, error) {
	...
	if len(d.transactions) > 0 {
		tt.id = len(d.transactions) + 1
		tt.pid = d.transation.id
	} else {
		d.transactions = make(map[int]*Transation)
		tt.id = 1
		tt.pid = 0
	}
	d.transation = tt
	d.transactions[tt.id] = tt
	return d, err
}
...
func (d *DB) Commit() (err error) {
	...
	if d.transation.id == 1 {
		...
		d.transation = nil
		d.transactions = make(map[int]*Transation, 0)
	} else {
		d.transation.status = Commit
		d.transation = d.transactions[d.transation.pid]
	}
	return err
}
...
func query(d *DB, query string) (q *sql.Rows, err error) {
	...
	if strings.Contains(query, "?") {
		if d.transation != nil {
			q, err = d.transation.tx.Query(query, d.params...)
		} else {
			q, err = d.connection.Query(query, d.params...)
		}
	} else {
		if d.transation != nil {
			q, err = d.transation.tx.Query(query)
		} else {
...
func exec(d *DB, query string) (res sql.Result, err error) {
	...
	if strings.Contains(query, "?") {
		if d.transation != nil {
			res, err = d.transation.tx.Exec(query, d.params...)
		} else {
			res, err = d.connection.Exec(query, d.params...)
		}
	} else {
		if d.transation != nil {
			res, err = d.transation.tx.Exec(query)
		} else {
